...
\begin{proof}
  ...
  Next, consider the case when $w$ is a vertex of the outer face.\todo{Not really needed.}
\end{proof}
...
\begin{clm}\label{has_descendant}
  If $i\ge j$, $|L_j\cap V(G_C)|\ge 2$ and a vertex $v\in L_j$ is not $i$-redundant then $v$ has a $T$-descendant\todo{Define $T$-descendant} in $L_i$.
\end{clm}
